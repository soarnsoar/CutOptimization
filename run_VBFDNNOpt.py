...
def DrawMassDNNscoreSgnfc(confname,confpath,rel=True,filtered=True,AddMaxLine=True): ## rel -> relative significance, filtered -> skip cut  98% of bkg
    ...
    #print masslistbin
    #h=ROOT.TH2D(confname,confname,len(masslist),masslistbin,21,-0.025,1.025) ##x : mass, y=DNNscore, z=significance
...
def DrawMassMaxSgnfc(confname,confpath,filtered=True):
    ...
    os.system("mkdir -p plots/")
    c.SaveAs("plots/sgnfc_"+confname+suffix+".pdf")
...
def DrawMassMaxScore(confname,confpath,filtered=True):
    ...
    os.system("mkdir -p plots/")
    c.SaveAs("plots/maxdnnpoint_"+confname+suffix+".pdf")
